...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=400):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            with open(file_path, encoding="utf-8") as f:
                logger.info(f'file {file_path} opened')
                for line in tqdm(f, mininterval=5):
            
                    tokenized_text = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(line))

                    for i in range(0, len(tokenized_text), block_size-2): # Truncate in block of block_size
                        truncated = tokenizer.build_inputs_with_special_tokens(tokenized_text[i:i+block_size-2])
                        if len(truncated) < block_size:
                            truncated = truncated + tokenizer.convert_tokens_to_ids(['<PAD>']) * (block_size - len(truncated))
                        self.examples.append(truncated)
            # Note that we are loosing the last truncated example here for the sake of simplicity (no padding)
            # If your dataset is small, first you should loook for a bigger one :-) and second you
            # can change this behavior by adding (model specific) padding.
            logger.info(f"Processed {len(self.examples)} blocks with block size = {block_size}")
...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--train_data_file", default='data/patch_dataset_all_train_v2.raw', type=str, help="The input training data file (a text file)."
    )
    parser.add_argument(
        "--output_dir",
        default=None,
    ...
    parser.add_argument(
        "--eval_data_file",
        default='data/patch_dataset_all_dev_v2.raw',
        type=str,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        if args.eval_all_checkpoints:
            ...
            logging.getLogger("transformers.configuration_utils.py").setLevel(logging.WARN)  # Reduce logging
        ...
        for checkpoint in checkpoints:
            ...
            result = dict((k + "_{}".format(global_step), v.item()) for k, v in result.items())
            results.update(result)
    print(results)
    print(sorted(results.items(), key=lambda x: x[-1]))
    with open(os.path.join(args.output_dir, "final_results.txt"), 'w') as outfile:
